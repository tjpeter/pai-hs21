...
class MyGPR(GaussianProcessRegressor):
    """
    Inheritance of the GaussianProcessRegressor in order to able to tune the optimizer for improved convergence
    Source: https://stackoverflow.com/a/62380182
    """
...
class BO_algo(object):
    def __init__(self, deg_exploration = 0.1):
        ...
        #The mapping c can be effectively modeled with as a product of a constant kernel with scale 3.5 
        #and an RBF kernel with lengthscale 2.
        #The observation noise is Gaussian with standard deviation σf=0.01 and σv=0.005 for the objective and the constraint, respectively.

        # degree of exploration, see https://arxiv.org/pdf/1807.01279.pdf
        self.deg_exploration = deg_exploration

        # TODO : GP model for the constraint function
        self.c_sigma = 0.005
        self.c_lengthscale = 2
        self.c_const = 3.5
        self.c_kernel = ConstantKernel(self.c_const,constant_value_bounds=(1e-6, 1e+05)) + RBF(length_scale=self.c_lengthscale,length_scale_bounds=(1e-6, 1e+05))
        self.constraint_model = MyGPR(
            kernel=self.c_kernel,
            alpha=self.c_sigma**2
        )
        # The mapping f can be effectively modeled with as a product of a constant kernel with 
        #scale 1.5 and an RBF kernel with lengthscale 1.5.
        # TODO : GP model for your acquisition function
        self.f_sigma = 0.01
        self.f_lengthscale = 1.5
        self.f_const = 1.5
        self.f_kernel = ConstantKernel(self.f_const,constant_value_bounds=(1e-6, 1e+05)) + RBF(length_scale=self.f_lengthscale,length_scale_bounds=(1e-6, 1e+05))
        self.objective_model = MyGPR(
    ...
    def next_recommendation(self) -> np.ndarray:
        ...
        if len(self.previous_points) == 0:
            xx = np.random.rand(domain_x.shape[0])*6
            next_r = np.array([xx])
        else:
            next_r = self.optimize_acquisition_function()
        return next_r
    ...
    def constraint(self, x):
        """
        Probability that constrained <=0 is satisfied
        """

        mu, sigma = self.constraint_model.predict([x], return_std=True)

        pr = norm.cdf(0, loc=mu, scale=sigma)
        return pr
    
    def exp_imp(self, x):
        """
        expected improvement, see Formula (2) in Gelbart, https://www.cs.princeton.edu/~rpa/pubs/gelbart2014constraints.pdf
        with an additional parameter to tune (for more) exploration according to https://arxiv.org/pdf/1807.01279.pdf
        
        """
        deg_exploration = self.deg_exploration
        mu, sigma = self.objective_model.predict([x], return_std=True)
        previous_points_arr=np.array(self.previous_points)
        mu_sample = self.objective_model.predict(previous_points_arr[:,:2])

        mu_min = np.min(mu_sample)
        z = (mu_min - (mu + deg_exploration))/ sigma
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        # TODO: enter your code here
        ei = self.exp_imp(x)
        constr = self.constraint(x)
        af_value=float(ei * constr)
        return af_value
    ...
    def add_data_point(self, x: np.ndarray, z: float, c: float):
        ...
        previous_points_arr=np.array(self.previous_points)
        #print(previous_points_arr)
        # TODO: enter your code here
        self.objective_model.fit(previous_points_arr[:,:2], previous_points_arr[:,2])
        self.constraint_model.fit(previous_points_arr[:,:2], previous_points_arr[:,3])
    ...
    def get_solution(self) -> np.ndarray:
        ...
        # TODO: enter your code here
        previous_points_arr=np.array(self.previous_points)
        results = previous_points_arr[:,2]
        results[previous_points_arr[:,3]> 0] = 1e5
        best_index = np.argmin(results)     
        x_opt = previous_points_arr[:,:2][best_index]
        print(previous_points_arr)
        return x_opt
