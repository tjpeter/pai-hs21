...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            action_distribution = self.pi._distribution(state)
            action = action_distribution.sample()
            log_p_action = self.pi._log_prob_from_distribution(action_distribution, action)
            value = self.v.forward(state)

        return action.item(), value.item(), log_p_action.item()
...
class Agent:
    ...
    def pi_update(self, data):
        ...
        dist = self.ac.pi._distribution(obs)
        logp = self.ac.pi._log_prob_from_distribution(dist, act)
    ...
    def get_action(self, obs):
        ...
        action = self.ac.pi._distribution(torch.as_tensor(obs, dtype=torch.float32)).sample()
