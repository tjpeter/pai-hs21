...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.weights_prior = MultivariateDiagonalGaussian(torch.Tensor(self.out_features,self.in_features).uniform_(-1,1),torch.Tensor(self.out_features,self.in_features).uniform_(-2,-1))
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        log_prior = self.weights_prior.log_likelihood(weights) + self.bias_prior.log_likelihood(bias)
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for idx, current_layer in enumerate(self.layers):
            forward_layer = current_layer(current_features)
